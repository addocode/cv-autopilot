...
function html() {
  ...
  const expHtml = cv.experiences.map((experience) => {
    ...
    const titleHtml = experience.id === 'mediamatiker-ausbildung-army-bict'
      ? `<div class="meta experience-title combined-training-title" data-source-ids="${esc((experience.sources || []).join(','))}" data-evidence-status="verified"><span data-ats-required data-ats-text="Mediamatiker EFZ in Ausbildung">Mediamatiker EFZ in Ausbildung</span><span class="combined-training-credential" data-ats-required data-ats-text="Berufsmaturität Wirtschaft und Dienstleistungen, Typ Wirtschaft">Berufsmaturität Wirtschaft und Dienstleistungen, Typ Wirtschaft</span></div>`
...
async function withPlaywright() {
  ...
  const metrics = await page.evaluate(({ bgExists, variantMeta }) => {
    ...
    out.experienceQuality.combinedTrainingCredential = { experienceId: 'mediamatiker-ausbildung-army-bict', primaryTitle: 'Mediamatiker EFZ in Ausbildung', credentialTitle: 'Berufsmaturität Wirtschaft und Dienstleistungen, Typ Wirtschaft', visible: Boolean(combined && combined.getClientRects().length), atsExtractable: Boolean(combined?.querySelector('[data-ats-text="Mediamatiker EFZ in Ausbildung"]') && combined?.querySelector('[data-ats-text="Berufsmaturität Wirtschaft und Dienstleistungen, Typ Wirtschaft"]')), sourceIds: (combined?.dataset.sourceIds || '').split(',').filter(Boolean), evidenceStatus: combined?.dataset.evidenceStatus || '', fontFamily: combinedStyle?.fontFamily || '', primaryFontFamily: getPrimaryFontFamily(combinedStyle?.fontFamily || ''), fontWeight: combinedStyle?.fontWeight || '' };
