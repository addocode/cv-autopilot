...
export function joinPdfTextItems(items, stats = {}) {
  ...
  stats.preservedSpaceCount ??= 0;
  stats.fragmentJoinExamples ??= [];
  stats.preservedSpaceExamples ??= [];
  ...
  for (const item of items) {
    ...
    const outputEndsWithSpace = /\s$/.test(output);
    const alreadySeparated = currentStartsWithSpace || outputEndsWithSpace;

    if (realLineBreak) {
      output += `\n${text}`;
      stats.insertedLineBreakCount += 1;
    } else if (alreadySeparated) {
      const nextText = outputEndsWithSpace && currentStartsWithSpace ? text.replace(/^\s+/, '') : text;
      output += nextText;
      stats.preservedSpaceCount += 1;
      if (stats.preservedSpaceExamples.length < 12) {
        stats.preservedSpaceExamples.push({
          before: [String(previous.str || ''), text],
          after: `${String(previous.str || '')}${nextText}`,
          separatorSource: currentStartsWithSpace ? 'current-item-leading-space' : 'previous-output-trailing-space',
        });
      }
    } else if (horizontalGap > wordGapThreshold) {
      output += ` ${text}`;
