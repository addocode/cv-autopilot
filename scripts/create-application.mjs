...
if (report.success !== true || report.pageCount !== 2 || report.overflows?.length || report.collisions?.length || report.warnings?.length || report.ats?.missingTerms?.length || report.selectedVariant !== selectedVariant) throw new Error('Application render report failed required gates');
...
manifest.archive.sha256 = sha(readFileSync(archivePath));
writeFileSync(join(dir, '04_manifest.json'), JSON.stringify(manifest, null, 2));
const tar2 = spawnSync('tar', ['--sort=name', '--mtime=@0', '--owner=0', '--group=0', '--numeric-owner', '-czf', archivePath, '-C', 'applications', applicationId], { encoding: 'utf8' });
if (tar2.status !== 0) throw new Error(`Export archive refresh failed.\n${tar2.stderr}`);
manifest.archive.sha256 = sha(readFileSync(archivePath));
writeFileSync(join(dir, '04_manifest.json'), JSON.stringify(manifest, null, 2));
console.log(JSON.stringify({ applicationId, directory: dir, archive: archivePath, archiveSha256: manifest.archive.sha256, selectedVariant }, null, 2));
